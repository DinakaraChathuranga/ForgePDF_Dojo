...
ipcMain.handle('image-to-pdf', async (event, imagePaths) => {
    const defaultPath = path.join(os.homedir(), 'Downloads', 'images_converted.pdf');
    const { filePath: savePath } = await dialog.showSaveDialog({ defaultPath });
    if (!savePath) return { success: false, message: 'Save cancelled.' };
    return runPythonScript('image_to_pdf.py', [imagePaths.join(','), savePath]);
});

ipcMain.handle('extract-pdf-text', async (event, filePath) => {
    return runPythonScript('edit_text.py', ['extract', filePath]);
});

ipcMain.handle('replace-pdf-text', async (event, filePath, replacementsJson) => {
    const defaultPath = path.join(os.homedir(), 'Downloads', `edited_${path.basename(filePath)}`);
    const { filePath: savePath } = await dialog.showSaveDialog({ defaultPath });
    if (!savePath) return { success: false, message: 'Save cancelled.' };
    return runPythonScript('edit_text.py', ['replace', filePath, replacementsJson, savePath]);
});
