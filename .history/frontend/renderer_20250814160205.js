document.addEventListener('DOMContentLoaded', () => {
    ...
    const imagesHandler = setupFileHandling('images', { isMultiple: true, isInteractive: true, acceptedFiles: '.png,.jpg,.jpeg' });
    ...
    const actionButtons = [
        ...
        { id: 'images-btn', handler: imagesHandler, api: 'imageToPDF', loadingMsg: 'Converting images...' }
    ];
        ...
        if (button) {
            button.addEventListener('click', async () => {
                try {
                    ...
                    if (api === 'mergePDFs') {
                        args = [files];
                    } else if (api === 'imageToPDF') {
                        args = [files]; // Pass the array directly
                    } else {
                        args = [files[0]];
                    }
    ...
    const editWorkspace = {
        ...
        setMode(newMode) {
            ...
            if (newMode === 'organize') {
                ...
                this.elements.modeInfo.textContent = "Click on text to edit it. Changes will be applied when you save.";
                const deleteBtns = this.elements.content.querySelectorAll('.delete-btn');
                deleteBtns.forEach(btn => btn.style.display = 'none');
                this.elements.modeControls.classList.add('visible');
                this.elements.modeControls.innerHTML = `
                    <button id="extract-text-btn" class="btn btn-sm btn-primary">Extract Text</button>
                    <div id="text-editor" style="display: none;">
                        <textarea id="text-content" placeholder="Edit text here..." rows="4" style="width: 100%; margin: 8px 0;"></textarea>
                    </div>
                `;
                
                document.getElementById('extract-text-btn').addEventListener('click', () => this.extractText());
                this.elements.saveBtn.innerHTML = '<i data-lucide="save"></i><span>Save Text Changes</span>';
            }
            lucide.createIcons();
        },
            ...
            try {
                if (this.state.mode === 'organize') {
                    ...
                    if (!this.state.textBlocks) {
                        return handleResponse({ success: false, message: 'Please extract text first.' });
                    }
                    
                    const newText = document.getElementById('text-content').value;
                    const oldText = this.state.textBlocks.map(block => block.text).join('\n');
                    
                    if (newText === oldText) {
                        return handleResponse({ success: false, message: 'No changes detected.' });
                    }
                    
                    // Create replacements (simplified - you might want more sophisticated diffing)
                    const replacements = [{
                        page: 1,
                        old_text: oldText,
                        new_text: newText,
                        bbox: this.state.textBlocks[0].bbox,
                        size: this.state.textBlocks[0].size,
                        color: this.state.textBlocks[0].color
                    }];
                    
                    showLoading('Saving text changes...');
                    result = await window.electronAPI.replacePdfText(this.state.currentFile, JSON.stringify(replacements));
                }
        ...
        async extractText() {
            try {
                ...
                const result = await window.electronAPI.extractPdfText(this.state.currentFile);
                if (result.success) {
                    this.state.textBlocks = result.text_blocks;
                    this.showTextEditor();
                } else {
                    handleResponse(result);
                }
        ...
        showTextEditor() {
            ...
            // Show extracted text for editing
            const textContent = this.state.textBlocks.map(block => block.text).join('\n');
