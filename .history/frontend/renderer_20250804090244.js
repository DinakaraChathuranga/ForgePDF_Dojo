document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupFileHandling(pageId, { isMultiple = false, isInteractive = false } = {}) {
        ...
        // Ensure you have the full setupFileHandling function from the previous step here
    }
    // setupFileHandling('merge', { isMultiple: true, isInteractive: true });
    // setupFileHandling('compress');
    // setupFileHandling('protect');
    // Add event listeners for merge, compress, protect buttons

    // --- NEW UNIFIED EDIT WORKSPACE ---
    const editState = {
        currentFile: null,
        mode: 'organize', // Default mode
        pages: [], // { element, originalIndex, isDeleted }
    ...
    function resetEditWorkspace() {
        ...
        editState.pages = [];
        editState.selectedForSplit.clear();
    ...
    async function handleEditFile(files) {
        ...
        if (result.success) {
            result.filePaths.forEach((path, i) => {
                ...
                editState.pages.push({ element: thumb, originalIndex: i + 1, isDeleted: false });
            });
    ...
    function createPageThumbnail(imagePath, pageNum) {
        ...
        thumb.innerHTML = `
            <img src="${imagePath.replace(/\\/g, '/')}?t=${new Date().getTime()}" alt="Page ${pageNum}" draggable="false">
            <span class="page-number">${pageNum}</span>
            <button class="delete-btn" style="display: none;"><i data-lucide="x"></i></button>
        `;
        // Re-initialize icons for the new element
        setTimeout(() => lucid.createIcons({
            attrs: {
                'stroke-width': 2.5,
                width: 16,
                height: 16,
            },
            nodes: [thumb.querySelector('.delete-btn')]
        }), 0);
        return thumb;
    }
    ...
    editToolbar.addEventListener('click', (e) => {
        ...
        if (toolBtn) {
            setEditMode(toolBtn.dataset.mode);
        }
    });
    ...
    function setEditMode(newMode) {
        ...
        editState.pages.forEach(p => {
            p.element.classList.remove('selected', 'marked-for-deletion');
            p.isDeleted = false;
        });
        editState.selectedForSplit.clear();
        ...
        if (newMode === 'organize') {
            editModeInfo.textContent = 'Drag to reorder, click X to mark for deletion.';
            deleteBtns.forEach(btn => btn.style.display = 'flex');
            editSaveBtn.disabled = false;
        } else if (newMode === 'split') {
            editModeInfo.textContent = 'Click pages to select them for the new file.';
            deleteBtns.forEach(btn => btn.style.display = 'none');
            editSaveBtn.disabled = true; // Disable until a page is selected
        } else if (newMode === 'rotate') {
            editModeInfo.textContent = 'Select a rotation angle to apply to all pages.';
            deleteBtns.forEach(btn => btn.style.display = 'none');
            editSaveBtn.disabled = false;
        }
    }

    // --- Event Handling for Different Modes ---
    editorContent.addEventListener('click', (e) => {
        ...
        if (editState.mode === 'organize') {
            if (e.target.closest('.delete-btn')) {
                const page = editState.pages.find(p => p.originalIndex === pageNum);
                page.isDeleted = !page.isDeleted;
                thumb.classList.toggle('marked-for-deletion', page.isDeleted);
            }
        } else if (editState.mode === 'split') {
            thumb.classList.toggle('selected');
            if (editState.selectedForSplit.has(pageNum)) {
                editState.selectedForSplit.delete(pageNum);
            } else {
                editState.selectedForSplit.add(pageNum);
            }
            editSaveBtn.disabled = editState.selectedForSplit.size === 0;
            editModeInfo.textContent = `${editState.selectedForSplit.size} page(s) selected for splitting.`;
        }
    });
    ...
    editorContent.addEventListener('dragover', (e) => {
        ...
        if (target && target !== draggedItem) {
            ...
            // True if dragging over the second half of the target
            const next = (e.clientX - rect.left) / rect.width > 0.5;
            if (next) {
                editorContent.insertBefore(draggedItem, target.nextSibling);
            } else {
    ...
    editSaveBtn.addEventListener('click', async () => {
        ...
        if (editState.mode === 'organize') {
            ...
            const pagesToSplit = Array.from(editState.selectedForSplit).sort((a,b) => a-b).join(',');
            result = await window.electronAPI.splitPDF(editState.currentFile, pagesToSplit);
             ...
             const { value: angle } = await Swal.fire({
                ...
                input: 'select',
                inputOptions: { 90: '90° Clockwise', 180: '180°', 270: '270° Clockwise (or 90° Anti-Clockwise)' },
                inputPlaceholder: 'Select an angle',
                showCancelButton: true,
            });
            if (angle) {
                 result = await window.electronAPI.rotatePDF(editState.currentFile, angle);
            } else {
                Swal.close();
        ...
        if (result && handleResponse(result)) {
            // On success, reset the workspace
            resetEditWorkspace();
        } else if (result) {
            Swal.close(); // Close loading spinner on failure
        }
    });
