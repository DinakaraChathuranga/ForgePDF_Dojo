document.addEventListener('DOMContentLoaded', () => {
    ...
    const organizeHandler = setupFileHandling('organize', { onFileLoad: setupOrganize('organize') });
    ...
    document.getElementById('organize-btn').addEventListener('click', async () => {
        const pageOrder = Array.from(document.querySelectorAll('#organize-thumbnail-grid .thumbnail-item')).map(item => item.dataset.page);
        const pagesToDelete = Array.from(document.querySelectorAll('#organize-thumbnail-grid .thumbnail-item.deleted')).map(item => item.dataset.page);
        
        showLoading('Organizing PDF...');
        const res = await window.electronAPI.organizePDF(organizeHandler.getFiles()[0], pageOrder, pagesToDelete);
        handleResponse(res);
        if (res.success) organizeHandler.reset();
    });
});

function setupOrganize(pageId) {
    const gridContainer = document.getElementById(`${pageId}-grid-container`);
    const dropArea = document.getElementById(`${pageId}-drop-area`);
    const grid = document.getElementById(`${pageId}-thumbnail-grid`);

    let currentFile = null;

    const renderThumbnails = async () => {
        grid.innerHTML = 'Loading thumbnails...';
        const result = await window.electronAPI.getPdfPreview(currentFile, -1); // -1 to get all pages
        if (result.success) {
            grid.innerHTML = '';
            result.filePaths.forEach((filePath, i) => {
                const item = document.createElement('div');
                item.className = 'thumbnail-item';
                item.dataset.page = i + 1;
                item.setAttribute('draggable', true);
                item.innerHTML = `
                    <img src="${filePath.replace(/\\/g, '/')}?t=${new Date().getTime()}" class="thumbnail-image">
            ...
            grid.innerHTML = 'Failed to load thumbnails.';
            handleResponse(result);
        }
    };
    ...
    grid.addEventListener('dragstart', (e) => {
        dragSrcEl = e.target.closest('.thumbnail-item');
        e.dataTransfer.effectAllowed = 'move';
        dragSrcEl.classList.add('dragging');
    });
    grid.addEventListener('dragend', () => dragSrcEl.classList.remove('dragging'));
