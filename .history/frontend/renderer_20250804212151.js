document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupFileHandling(pageId, { isMultiple = false, isInteractive = false, acceptedFiles = '.pdf' } = {}) {
        ...
        const updateUI = () => {
            ...
            if (pageId === 'protect') {
                document.getElementById('toggle-password-btn').disabled = !isReady;
            }
        };

        const handleFiles = (newFiles) => {
            const acceptedTypes = acceptedFiles.split(',').map(t => t.trim());
            const filteredFiles = Array.from(newFiles).filter(f => acceptedTypes.some(type => f.name.endsWith(type) || f.type.startsWith(type.replace('.', ''))));
    ...
    document.getElementById('image-to-pdf-btn').addEventListener('click', async () => { showLoading('Converting to PDF...'); const res = await window.electronAPI.imageToPdf(imageToPdfHandler.getFiles()); if (handleResponse(res)) imageToPdfHandler.reset(); });
    document.getElementById('pdf-to-image-btn').addEventListener('click', async () => { showLoading('Converting to Images...'); const res = await window.electronAPI.pdfToImage(pdfToImageHandler.getFiles()[0]); if (handleResponse(res)) pdfToImageHandler.reset(); });
