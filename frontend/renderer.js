document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupFileHandling(pageId, isMultiple) {
        ...
        const updateUI = () => {
            ...
            if(pageId === 'watermark') { ['watermark-text', 'watermark-orientation', 'watermark-size'].forEach(id => document.getElementById(id).disabled = !isReady); }
        };
    ...
    document.getElementById('watermark-btn').addEventListener('click', async () => {
        const options = { text: document.getElementById('watermark-text').value, orientation: document.getElementById('watermark-orientation').value, size: document.getElementById('watermark-size').value };
        if (!options.text) { Swal.fire({ icon: 'warning', title: 'Text Required' }); return; }
        showLoading('Adding Watermark...');
        const result = await window.electronAPI.watermarkPDF(watermarkHandler.getFiles()[0], options);
        handleResponse(result);
        if (result.success) { watermarkHandler.reset(); document.getElementById('watermark-text').value = ''; }
    });
